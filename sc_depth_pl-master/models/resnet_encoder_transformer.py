...
# input transformer by HuangYanhao   2023/3/26
...
class ResnetEncoder(nn.Module):
    def __init__(self, num_layers, pretrained, num_input_images=1):
        ...
        # 添加Transformer层
        self.transformer_layer = TransformerEncoderLayer(d_model=self.num_ch_enc[-1], nhead=4)
    ...
    def forward(self, input_image):
        ...
        # 在最后一个resnet层之后添加Transformer层
        bs, _, h, w = x.size()
        x = x.view(bs, self.num_ch_enc[-1], -1).permute(0, 2, 1)
        x = self.transformer_encoder(x)
        x = x.permute(0, 2, 1).view(bs, self.num_ch_enc[-1], h, w)
