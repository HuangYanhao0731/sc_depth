...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, num_input_images=1):
        ...
        # 引入新的ViT编码器实例
        self.vit_encoder = vision_transformer.vit_base_patch16_224(pretrained=True)
    ...
    def forward(self, input_image):
        ...
        # 将输入图像分割成patch并输入到ViT编码器
        vit_features = self.vit_encoder(input_image)
        ...
        # 融合ViT编码器和ResNet编码器的特征
        for i in range(len(self.features)):
            self.features[i] = self.features[i] + vit_features[i]
