...
from .resnet_encoder_transformer import ResnetEncoder
# 加入Transformer
...
class DepthDecoder(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=1, use_skips=True):
        ...
        for i in range(4, -1, -1):
            trans_layer = TransformerEncoderLayer(d_model=self.num_ch_dec[i], nhead=4)
            self.transformer_layers.append(TransformerEncoder(trans_layer, num_layers=2))
    ...
    def forward(self, input_features):
        ...
        for cnt, i in enumerate(range(4, -1, -1)):
            ...
            bs, _, h, w = x.size()
            x = x.view(bs, self.num_ch_dec[i], -1).permute(0, 2, 1)
            x = self.transformer_layers[cnt](x)
            x = x.permute(0, 2, 1).view(bs, self.num_ch_dec[i], h, w)
